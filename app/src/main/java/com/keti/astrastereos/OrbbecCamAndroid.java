...
public class OrbbecCamAndroid {
    ...
    public ArrayList<Vector3D> getDepthData() {
        ...
        try {
            ...
            //while (true) {
                Astra.update();
                TimeUnit.MILLISECONDS.sleep(updateSleepMS);
                //print_depth(df, depthStream);

                //cnt++;
                 //Log.d(logTagDepthStream, "[CHA] size of list: " + vector3DList.size());
                //if (cnt == maxCnt) break;
            }
    ...
    public void print_depth(DepthFrame df, DepthStream ds) {
        if (df.isValid()) {
            int width = df.getWidth();
            int height = df.getHeight();
            Log.d("DEPTH", "WIDTH: " + width);
            Log.d("DEPTH", "HEIGHT: " + height);
        }
    }
